...
def generate_table1_agreement_metrics(data_dir: str, output_dir: str):
    ...
    table_data = {
        ...
        'p-value': [f"{pearson_p:.4f}" if pearson_p > 0.001 else "<0.001",
                    f"{spearman_p:.4f}" if spearman_p > 0.001 else "<0.001",
                    "<0.001"]
    }

    df = pd.DataFrame(table_data)
    df.to_csv(os.path.join(output_dir, "table1_sensor_reference_agreement.csv"), index=False)
...
\hline
Pearson correlation ($r$) & """ + f"{pearson_r:.4f}" + r""" & $<0.001$ \\
Spearman correlation ($\rho$) & """ + f"{spearman_r:.4f}" + r""" & $<0.001$ \\
ICC(2,1) & """ + f"{icc:.4f}" + r""" & $<0.001$ \\
\hline
\end{tabular}
\end{table}
"""

    with open(os.path.join(output_dir, "table1_sensor_reference_agreement.tex"), 'w') as f:
        f.write(latex)
...
def generate_table2_error_analysis(data_dir: str, output_dir: str):
    ...
    table_data = {
        'Metric': ['Mean Absolute Error (MAE)', 'Root Mean Square Error (RMSE)',
                   'Bias (Mean Error)', 'Standard Error', 'Median Absolute Error',
                   'Mean Absolute Percentage Error (MAPE)'],
        'Value': [f"{mae:.4f}", f"{rmse:.4f}", f"{bias:.4f}", f"{std_err:.4f}",
                  f"{median_ae:.4f}", f"{mape:.2f}\\%"],
        'Unit': ['kg', 'kg', 'kg', 'kg', 'kg', '\\%']
    }
...
\hline
Mean Absolute Error (MAE) & """ + f"{mae:.4f}" + r""" & kg \\
Root Mean Square Error (RMSE) & """ + f"{rmse:.4f}" + r""" & kg \\
Bias (Mean Error) & """ + f"{bias:.4f}" + r""" & kg \\
Standard Error & """ + f"{std_err:.4f}" + r""" & kg \\
Median Absolute Error & """ + f"{median_ae:.4f}" + r""" & kg \\
Mean Absolute Percentage Error & """ + f"{mape:.2f}" + r"""\% & \% \\
\hline
\end{tabular}
\end{table}
"""
...
def generate_table3_bland_altman(data_dir: str, output_dir: str):
    ...
    prop_bias_slope = ba_df[ba_df['metric'] == 'proportional_bias_slope']['value'].values[0]
    prop_bias_p = ba_df[ba_df['metric'] == 'proportional_bias_p_value']['value'].values[0]
    ...
    table_data = {
        'Parameter': ['Bias (mean difference)', 'Standard deviation of differences',
                      'Upper LoA (bias + 1.96×SD)', 'Lower LoA (bias - 1.96×SD)',
                      'Points outside LoA (%)', 'Proportional bias slope',
                      'Proportional bias p-value'],
        'Value': [f"{bias:.4f}", f"{sd_diff:.4f}", f"{upper_loa:.4f}",
                  f"{lower_loa:.4f}", f"{pct_outside:.2f}",
                  f"{prop_bias_slope:.4f}", f"{prop_bias_p:.4f}"],
...
\hline
Bias (mean difference) & """ + f"{bias:.4f}" + r""" & """ + f"{bias:.4f}" + r""" $\pm$ """ + f"{ci_bias:.6f}" + r""" \\
Standard deviation of differences & """ + f"{sd_diff:.4f}" + r""" & -- \\
Upper LoA (bias + 1.96$\times$SD) & """ + f"{upper_loa:.4f}" + r""" & -- \\
Lower LoA (bias - 1.96$\times$SD) & """ + f"{lower_loa:.4f}" + r""" & -- \\
Points outside LoA (\%) & """ + f"{pct_outside:.2f}" + r""" & -- \\
Proportional bias slope & """ + f"{prop_bias_slope:.4f}" + r""" & -- \\
Proportional bias p-value & """ + f"{prop_bias_p:.4f}" + r""" & -- \\
\hline
\end{tabular}
\end{table}
"""
...
def generate_table4_paired_comparison(data_dir: str, output_dir: str):
    ...
    table_data = {
        'Test': ['Shapiro-Wilk Normality (W)', 'Shapiro-Wilk p-value',
                 'Paired t-test (t)', 'Paired t-test p-value'],
        'Value': [f"{shapiro_w:.4f}", f"{shapiro_p:.4f}",
                  f"{t_stat:.4f}", f"{t_p:.4f}"],
        'Interpretation': ['Normal distribution' if normality == 'Yes' else 'Non-normal',
    ...
    df = pd.DataFrame(table_data)
    df.to_csv(os.path.join(output_dir, "table4_statistical_comparison.csv"), index=False)
...
\hline
Shapiro-Wilk Normality ($W$) & """ + f"{shapiro_w:.4f}" + r""" & """ + ('Normally distributed' if normality == 'Yes' else 'Non-normal distribution') + r""" \\
Shapiro-Wilk p-value & """ + f"{shapiro_p:.4f}" + r""" & $p """ + (">" if normality == 'Yes' else r"\leq") + r""" 0.05$ \\
Paired t-test ($t$) & """ + f"{t_stat:.4f}" + r""" & """ + ('No significant difference' if sig_diff == 'No' else 'Significant difference') + r""" \\
Paired t-test p-value & """ + f"{t_p:.4f}" + r""" & $p """ + ("<" if sig_diff == 'Yes' else r"\geq") + r""" 0.05$ \\
\hline
\end{tabular}
\end{table}
"""
...
def generate_combined_summary_table(data_dir: str, output_dir: str):
    ...
    table_data = {
        'Category': ['Correlation', 'Correlation', 'Agreement',
                     'Error', 'Error', 'Error', 'Error',
                     'Bland-Altman', 'Bland-Altman', 'Bland-Altman'],
        'Metric': ['Pearson r', 'Spearman ρ', 'ICC(2,1)',
                   'MAE', 'RMSE', 'Bias', 'MAPE',
                   'Upper LoA', 'Lower LoA', '% Outside LoA'],
        'Value': [f"{pearson_r:.4f}", f"{spearman_r:.4f}", f"{icc:.4f}",
                  f"{mae:.4f}", f"{rmse:.4f}", f"{bias:.4f}", f"{mape:.2f}%",
                  f"{upper_loa:.4f}", f"{lower_loa:.4f}", f"{pct_outside:.2f}%"]
    }
