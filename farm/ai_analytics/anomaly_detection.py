...
class AnomalyDetector:
    ...
    def __init__(self, contamination=0.05, random_state=42):
        ...
        self.lof = None
        self.scaler = StandardScaler()
        self.z_threshold = 2.5   # 2.5-sigma against clean baseline (3.0 was too strict)
    ...
    def predict_zscore(self, df):
        """
        Predict anomalies using baseline z-score (mean/std from clean training data).

        Uses training-set statistics rather than a rolling window over the test data,
        which prevents injected anomalies from inflating the local std and masking
        themselves — the classic contamination-bias problem with rolling z-scores.

        Returns:
            z_labels: binary labels (1 = anomaly)
            z_scores: dict with per-sensor z-scores
        """
        z_scores = {}
        ...
        for sensor in self.sensor_cols:
            ...
            std  = max(self.baseline_stats[sensor]['std'], 1e-9)
            z = np.abs((df[sensor].values - mean) / std)
            z_scores[sensor] = z
            sensor_anomalies = z > self.z_threshold
    ...
    def predict_ensemble(self, df, voting='soft', threshold=0.45):
        """
